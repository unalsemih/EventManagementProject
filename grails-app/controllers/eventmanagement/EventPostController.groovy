...
class EventPostController {
    ...
    def index(int id) {
     ...
     //   List<String,byte[]> userAvatar = []
        ...
        println(photos)
        Participation a = Participation.findByPostIdAndUsername(post.id, springSecurityService.getCurrentUser().username)
        def status=false
        if(a!=null)
            status=true
        render (view:"ePost",model: [currentUser:currentUser,post:post,user:user,eventPosts:eventPosts,images:photos,comments:comments,replies:replies,userAvatar:userAvatar,messageText:message,status:status]);
    }
